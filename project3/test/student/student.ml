...
let test_nfa_accept _ =
  let m1 =
    {qs= [0; 1]; sigma= ['a'; 'b']; delta= [(0, None, 1);(1, None, 0);(0, Some 'a', 1)]; q0= 0; fs= [1]}
  in
  let m2 =
    {qs= [0; 1; 2; 3; 4; 5]; sigma= ['a'; 'b'; 'c']; delta= [(0, Some 'a', 1);(0, Some 'b', 3);(3, Some 'c', 4);(1, Some 'b', 2);(4, Some 'a', 2);(2, Some 'c', 5);(5, Some 'c', 5);(5, None, 4)]; q0= 0; fs= [5]}
  in
  let m1DFA = nfa_to_dfa m1 in
  let m2DFA = nfa_to_dfa m2 in
  assert_dfa m2DFA ;
  assert_nfa_accept m2DFA "bcacacacc";
  assert_nfa_accept m1DFA "";
  assert_nfa_deny m1DFA "b";
  assert_nfa_accept m1DFA "a"
...
let test_str_to_nfa_empty _ =
  let m1 = regexp_to_nfa @@ string_to_regexp "(((E)|(E))*)|(ab)*" in
  assert_nfa_deny m1 "jujujuju";
  assert_nfa_accept m1 "ababab"
